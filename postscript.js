...
(function () {// node_modules/github-slugger/index.js
var own = Object.hasOwnProperty;
...
async function setupExplorer(currentSlug) {
  ...
  if (!explorerTree) {
    explorerTree = buildExplorerTree(await fetchData);
  }
...
document.addEventListener("nav", async () => {
  currentExplorerSlug = document.body.dataset.slug;
...
function setCollapsedByPath(array, path, collapsed) {
...
function buildExplorerTree(data) {
  const root = { name: "", displayName: "", children: [], depth: 0 };
  for (const slug2 of Object.keys(data)) {
    if (slug2 === "index" || slug2.startsWith("tags/")) {
      continue;
    }
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      if (isLast) {
        node.children.push({
          name,
          displayName: data[slug2]?.title ?? name,
          slug: slug2,
          children: [],
          depth: node.depth + 1
        });
...
function getFolderPaths(tree, collapsed) {
  const folderPaths = [];
...
function mergeFolderState(defaultState, storageTree) {
  if (!storageTree) {
    return defaultState;
  }
  try {
    const savedState = JSON.parse(storageTree);
...
function renderExplorer(tree, currentSlug) {
  const explorer = document.getElementById("explorer-ul");
  if (!explorer) {
    return;
  }
...
function renderNode(node, currentSlug, parentPath) {
