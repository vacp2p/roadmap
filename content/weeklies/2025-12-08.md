---
title: 2025-12-08 Vac weekly
tags:
  - vac-updates
date: 2025-12-08
lastmod: 2025-12-08
draft: false
---

## Vac 2025/12/08

### highlights
- DST: RLN tree scaling scenario now runs end-to-end on the new branch after token ratio adjustments, with final benchmarks queued.
- DST: lsquic regression testing shows improved latency; flagged an underperforming lab node for follow-up with Wings.
- DST: Browser bootstrapping client logging still fails to push (`No peer available`); config refactor PRs are underway.
- DST: nwaku v0.37 store retrieval and bandwidth regressions are under investigation with detailed Notion reports and Discord threads.
- DST: Vaclab local environment now excludes system namespaces, consolidates dashboards, and compares CNI stacks for the duplicate lab plan.
- Infra: Completed Fusaka upgrade/recovery work and shipped a postmortem while expanding Geth diversity and validator monitoring.
- Infra: Rolled out a new internal site inventory with improved Consul tagging and Watchtower upgrade.
- Infra: Added Linux and Mac M4 Jenkins hosts after debugging Netdata memory issues and Windows VM licensing research.
- Infra: Status CI now runs nwaku variants with improved Windows E2E stability and socket diagnostics.
- Infra: Nimbus fleets had storage cleanup, sudo fixes, and prep for unified binaries.
- Nescience: Documented privacy-preserving tail-call issues and proposed fixes for NSSA.
- Nescience: Summarized private PDA design decisions and extracted NSSA cryptographic primitives for alignment with Logos.
- Nescience: Advanced block-context designs and testnet readiness with multiple merged LSSA PRs plus wallet deployment tooling.
- Nescience: Pushed documentation updates and program deployment commands toward testnet readiness.
- Nim: Nimble 1.0 work continued with operator warning fixes and dependency log cleanup.
- Nim: Confutils received multiple fixes for help/version output and unsigned int parsing issues.
- Nim: intops gained 32-bit support, carrying operations, and tech design documentation to replace stint primitives.
- Nim: Langserver docs now include inlay hint instructions; small unittest2 doc tweak submitted.
- P2P: QUIC stabilization advanced with nat multi-port support, nimbus integration work, and lsquic assembly fixes across platforms.
- P2P: C-bindings core gained start/stop/connect APIs plus peerInfo; pubsub support (pt1) is in progress.
- P2P: Gossipsub robustness improvements landed across multiple PRs.
- P2P: 2026Q1 commitments draft is open for review.
- QA: Token-gated communities functional tests are under refactor with follow-up cases queued for merge.
- QA: nim-libp2p flaky-test cleanup progressed with multiple PRs and chronicles follow-ups opened.
- QA: Desktop QA shipped Windows E2E fixes and continued release testing; app load-time baselines are pending infra input.
- QA: Mobile QA progressed multi-device support, ported Appium tests, and advanced Jenkins/BrowserStack CI work.
- RFC: RFC index maintenance continued with review responses.
- RFC: Waku opchan RFC drafting progressed toward first review.
- RFC: Multiple Nomos RFC drafts advanced (message encapsulation/formatting, blend protocol, proof of quota, Codex prover, block exchange).
- SC: StakeManager and StakeVault maintenance fixes landed, including paused-leave handling and migration safeguards.
- SC: Audit finding reviews are ongoing with additional fixes planned (vault ownership limits, slasher penalties).
- Security: OS hardening guidance refreshed; secure signing training progressed with legacy member onboarding.
- Security: Cold signing environments advanced with Qubes OS deployment and offline verification planning.
- Security: Treasury continuity tasks expanded device assignments and signing account revalidation.
- Security: Status design reviews, npm malware sweeps, and CodeQL scans covered Waku/VAC/Status repos with dependency pinning and SBOM updates.
- TKE: Price prediction model updates continued with latest feedback addressed.
- TKE: Logos token digital twin modeling progressed with Machinations learning and stock/flow diagrams.
- TKE: Reading on virtual economies fed into broader unit support work.
- Web: SN Hub release PR is near merge with responsive fixes, new TVL displays, and staking/Karma flow updates.
- Web: Wallet extension proxy work progressed (price proxy UI reviews, token shard updates) with mainnet deposits tested.
- Web: Logos website streams gained calendar embeds, metrics fields, and CVE-driven Next.js upgrades across sites.
- Web: Maintenance included Waku shard changes for Communities and privacy policy updates; CI improvements underway for non-Vercel contributors.

### vac:dst:
- `admin/misc`
  - Investigated lab issues while running lsquic analysis and synced with Waku regression findings.
  - OOO: 5 CC days.
- [`vac:dst:vac:2025q4-libp2p-evaluation:regression-testing`](/dst/vac/2025q4-libp2p-evaluation#regression-testing-recurring)
  - Saw excellent lsquic results; documented findings and traced latency outliers to specific Kubernetes workers.
  - Underperforming worker escalated to Wings (temperature-related).
- [`vac:dst:waku:2025q4-waku-scaling:TWN-supports-RLN-tree`](/dst/waku/2025q4-waku-scaling#twn-supports-rln-onchain-tree-and-deposits-existing-memberships-only)
  - Switched to the new branch, adjusted LinearPriceCalculator token ratios, and validated the scenario end-to-end pending benchmark runs.
- [`vac:dst:ift:2025q4-dst-tooling:general-tooling`](/dst/ift/2025q4-dst-tooling#general-tooling-recurring)
  - [GitHub PR #179](https://github.com/vacp2p/10ksim/pull/179) merged to add builders for Waku store nodes.
- [`vac:dst:waku:2025q4-waku-scaling:optimise-browser-bootstrapping`](/dst/waku/2025q4-waku-scaling#optimise-browser-bootstrapping)
  - JsWaku client logging image still fails to push (`No peer available`); surfaced raw errors and continued config refactor in [pod-api-requester PR #2](https://github.com/vacp2p/pod-api-requester/pull/2).
- [`vac:dst:waku:2025q4-waku-evaluation:nwaku-v0.37-store-regression`](/dst/waku/2025q4-waku-evaluation#nwaku-v037-store-regression)
  - Ran regression experiments, identified store message retrieval gaps, and compiled findings in the [Get Store Messages Issue report](https://www.notion.so/Get-Store-Messages-Issue-2bb8f96fb65c80bc8e0fdb7801cd0c79) and [Discord thread](https://discord.com/channels/864066763682218004/1446533711028814048).
- [`vac:dst:waku:2025q4-waku-evaluation:nwaku-v0.37-bandwidth-regression`](/dst/waku/2025q4-waku-evaluation#nwaku-v037-bandwidth-regression)
  - Benchmarked bandwidth usage for nwaku v0.37; documented results in the [1K Relay + 3-Bootstrap Nodes report](https://www.notion.so/1K-Relay-3-Bootstrap-Nodes-2bb8f96fb65c8036b66ef85fff17c515) and [Discord thread](https://discord.com/channels/864066763682218004/1443602126034043071).
- [`vac:dst:ift:2025q4-dst-lab:local-lab`](/dst/ift/2025q4-dst-lab#local-lab)
  - [GitHub PR #2](https://github.com/vacp2p/vaclab-local/pull/2) excludes system namespaces, trims metrics, and imports dashboards; documented usage in the [Vaclab local guide](https://www.notion.so/Local-Vaclab-usage-information-2c38f96fb65c80dd9e7df7294fdbdf72).
- [`vac:dst:ift:2025q4-dst-lab:lab-duplication`](/dst/ift/2025q4-dst-lab#lab-duplication)
  - Compared CNI options (Antera, Calico, Cilium, KubeOVN) and explored combinations for isolation, monitoring, and dual-network pods.

### vac:infra:
- LIDO & ETH2
  - Fusaka Upgrade, Downtime, Recovery, and Postmortem
    - https://research.lido.fi/t/post-mortem-develp-fleet-fusaka-fork-downtime-2025-12-03/10967
  - Deployed more Geth nodes for better EL client diversity
  - Renamed hosts to match the new multi-EL layout
  - Debugging of Nethermind sync issues due to false invalid blocks
  - Reth and Lighthouse upgrades for validator watcher service
  - Merged runbook for debugging missed proposals
  - Work on LIDO v3 stVault infrastructure deployment
- HQ
  - Deployemtn of new internal site inventory
  - Improvements to Consul tags for internal site inventory
  - Watchtower upgrade to new fork with Docker API fixes
  - Fixed Onion site hosting Consul healthcheck script
  - Fixed inconsistent small SWAP sizes on Consul fleet
- CI
  - Debugging of memory issues caused by Netdata on CI hosts
  - Research into licensing for Windows E2E VMs
  - Added new Linux and MacOS M4 hosts to Jenkins CI
- STATUS
  - Setup of Nwaku versions of all `status-go` CI jobs
  - Impovements to Windows E2E test stability
  - Work on measuring Windows E2E test stage duration
  - Debugging socket issues on `notify.prod fleet
  - Renewed iOS certificates for legacy Status app
  - Discovered bad security practices in new mobile app setup
- NIMBUS
  - Cleanup of Nimbus Portal and Mainnet fleet storage issues
  - Fixed Sudo configuration for Nimbus devs on fleets
- SITES
  - Debugging of reported Discourse registration issues
  - Improvements to Keycard Shell deployment setup
  - Deployed new Logos Ecosystem website
- PROXY
- MISC
  - Renaming of Codex, Waku, and Nomos orgs and repositories
  - Research into CiviCRM deployment for Logos

### vac:nescience:
- `state-separation-architecture-poc:privacy-tail-calls2`
  - Drafted summaries of tail-call issues in privacy-preserving transactions and proposed mitigation approaches.
- `state-separation-architecture-poc:pda-research`
  - Discussed private PDAs and captured decisions in a design summary.
- `state-separation-architecture-poc:crypto-primitives`
  - Catalogued NSSA cryptographic primitives, rationale, and flexibility for broader Logos alignment.
- `state-separation-architecture-poc:block-context`
  - Explored approaches to provide block context for NSSA programs (e.g., voting use cases).
- `state-separation-architecture-poc:testnet-readiness`
  - Finalized [PR 211](https://github.com/logos-blockchain/lssa/pull/211), [PR 215](https://github.com/logos-blockchain/lssa/pull/215), [PR 216](https://github.com/logos-blockchain/lssa/pull/216), continued [PR 153](https://github.com/logos-blockchain/lssa/pull/153), opened [account claiming fix](https://www.notion.so/Only-the-outer-program-in-a-chain-of-calls-can-claim-accounts-2b78f96fb65c80b39bbaed9bda016f93), [wallet deployment command PR #221](https://github.com/logos-blockchain/lssa/pull/221), [piñata features PR #205](https://github.com/vacp2p/nescience-testnet/pull/205), [wallet sync fix PR #217](https://github.com/logos-blockchain/lssa/pull/217), [account data limit PR #220](https://github.com/logos-blockchain/lssa/pull/220), and [balance check fix PR #223](https://github.com/logos-blockchain/lssa/pull/223).
- `state-separation-architecture-poc:documentation`
  - Wrapped up the documentation task for the NSSA PoC.

### vac:nim:
- [`vac:nim:ift:2025q4-nimble:nimble-1.0.0`](/nim/ift/2025q4-nimble#nimble-100)
  - [M] [nimble PR #1530](https://github.com/nim-lang/nimble/pull/1530) fixes the `^=` version operator warning; [M] [nimble PR #1529](https://github.com/nim-lang/nimble/pull/1529) cleans dependency logs; initial work to support nwaku as a library.
- [`vac:nim:ift:2025q4-nim-core-libs:command-line-parsers`](/nim/ift/2025q4-nim-core-libs#command-line-parsers)
  - [O] [nim-confutils PR #128](https://github.com/status-im/nim-confutils/pull/128) addresses unsigned int wrapping; [O] [PR #127](https://github.com/status-im/nim-confutils/pull/127) restores `--help/--version`; [O] [PR #126](https://github.com/status-im/nim-confutils/pull/126) adds a debug tag; created a [Core Libs inventory](https://www.notion.so/Nim-dc915d763ce64c8486b41523dc67f65b).
- [`vac:nim:ift:2025q4-nim-core-libs:cpu-integer-library`](/nim/ift/2025q4-nim-core-libs#create-standalone-cpu-integer-library)
  - [M] [nim-intops PR #5](https://github.com/vacp2p/nim-intops/pull/5) adds 32-bit support and switches to unittest2; [O] [PR #8](https://github.com/vacp2p/nim-intops/pull/8) begins migrating stint primitives with refactors and design docs; [O] [PR #7](https://github.com/vacp2p/nim-intops/pull/7) adds carrying multiplication and muladd.
- `langserver`
  - [M/O] [langserver PR #369](https://github.com/nim-lang/langserver/pull/369) and [PR #370](https://github.com/nim-lang/langserver/pull/370) document inlay hints.
- `misc`
  - [M] [unittest2 PR #64](https://github.com/status-im/nim-unittest2/pull/64) improves docs; reported a nimble dependency install issue: [nimble issue #1531](https://github.com/nim-lang/nimble/issues/1531).

### vac:p2p:
- [`vac:p2p:ift:2025q4-nimlibp2p-quic:stabilization`](/p2p/ift/2025q4-nimlibp2p-quic#quic-stabilization)
  - [O] [nim-eth PR #834](https://github.com/status-im/nim-eth/pull/834) adds multi-port NAT; [O] [nimbus-eth2 PR #7775](https://github.com/status-im/nimbus-eth2/pull/7775) WIP QUIC; [M] [nim-lsquic PR #31](https://github.com/vacp2p/nim-lsquic/pull/31) adds Windows ASM; [M] [PR #29](https://github.com/vacp2p/nim-lsquic/pull/29) uses assembly on Linux/mac; [M] [PR #27](https://github.com/vacp2p/nim-lsquic/pull/27) refactors native UDP; [M] [PR #30](https://github.com/vacp2p/nim-lsquic/pull/30) caches client certs; [M] [PR #26](https://github.com/vacp2p/nim-lsquic/pull/26) simplifies timers/locks.
- [`vac:p2p:ift:2025q4-nimlibp2p-cbindings:core`](/p2p/ift/2025q4-nimlibp2p-cbindings#core-bindings)
  - [O] [nim-libp2p PR #1927](https://github.com/vacp2p/nim-libp2p/pull/1927) adds start/stop/connect/disconnect; [O] [PR #1931](https://github.com/vacp2p/nim-libp2p/pull/1931) adds peerInfo.
- [`vac:p2p:ift:2025q4-nimlibp2p-cbindings:gossipsub`](/p2p/ift/2025q4-nimlibp2p-cbindings#gossipsub)
  - [O] [nim-libp2p PR #1935](https://github.com/vacp2p/nim-libp2p/pull/1935) adds pubsub (pt1).
- `misc/admin`
  - [O] [roadmap PR #353](https://github.com/vacp2p/roadmap/pull/353) drafts 2026Q1 commitments.
- [`vac:p2p:ift:2025q4-nimlibp2p-quic:stabilization`](/p2p/ift/2025q4-nimlibp2p-quic#quic-stabilization)
  - [O] [nim-libp2p PR #1942](https://github.com/vacp2p/nim-libp2p/pull/1942) improves gossipsub tests; [O] [PR #1934](https://github.com/vacp2p/nim-libp2p/pull/1934) removes redundant calls; [M] [PR #1938](https://github.com/vacp2p/nim-libp2p/pull/1938), [PR #1936](https://github.com/vacp2p/nim-libp2p/pull/1936), [PR #1933](https://github.com/vacp2p/nim-libp2p/pull/1933), [PR #1930](https://github.com/vacp2p/nim-libp2p/pull/1930), [PR #1924](https://github.com/vacp2p/nim-libp2p/pull/1924) harden gossipsub tests and pubsub.

### vac:qa:
- `status:2025q4-status-go-functional-testing:wallet`
  - [O] [test: Token gated communities](https://github.com/status-im/status-go/pull/7113) under refactor; [C] [TC 7128](https://github.com/status-im/status-go/pull/7144) pending upstream resolution; [O] [issue #7167](https://github.com/status-im/status-go/issues/7167) on community changes visibility.
- [`vac:2025q4-nim-libp2p-testing:flaky-tests`](/qa/vac/2025q4-nim-libp2p-testing#flaky-test-fixes)
  - [O] [nim-libp2p PR #1939](https://github.com/vacp2p/nim-libp2p/pull/1939) organizes nimble files; [O] [issue #1941](https://github.com/vacp2p/nim-libp2p/issues/1941) and [issue #1940](https://github.com/vacp2p/nim-libp2p/issues/1940) track chronicles handling; [O] [PR #1929](https://github.com/vacp2p/nim-libp2p/pull/1929) ensures tests run without flags; [M] [PR #1932](https://github.com/vacp2p/nim-libp2p/pull/1932) reports only failed tests; [M] [PR #1928](https://github.com/vacp2p/nim-libp2p/pull/1928) stabilizes switch tests; [M] [PR #1918](https://github.com/vacp2p/nim-libp2p/pull/1918) dynamic imports; [M] [PR #1922](https://github.com/vacp2p/nim-libp2p/pull/1922) coverage ignores; [M] [PR #1920](https://github.com/vacp2p/nim-libp2p/pull/1920) log fixes.
- [`vac:qa:status:2025q4-status-qa-desktop:maintenance`](/qa/status/2025q4-status-qa-desktop#maintenance)
  - [M] [status-desktop PR #19406](https://github.com/status-im/status-desktop/pull/19406) Windows E2E fixes; [M] [PR #19386](https://github.com/status-im/status-desktop/pull/19386) retry popup handling.
- [`vac:qa:status:2025q4-status-qa-desktop:app-load-time`](/qa/status/2025q4-status-qa-desktop#measure-app-load-time)
  - Continued investigation and aligned with infra; awaiting baselines from Volo.
- [`vac:qa:status:2025q4-status-qa-desktop:release-testing`](/qa/status/2025q4-status-qa-desktop#release-testing)
  - Ongoing retests for [PR #18965](https://github.com/status-im/status-desktop/pull/18965).
- [`vac:qa:status:2025q4-status-qa-mobile:multi-device-support`](/qa/status/2025q4-status-qa-mobile#multi-device-support--parallel-sessions)
  - Awaiting review.
- `status:2025q4-status-qa-mobile:port-tests`
  - [O] [status-desktop PR #19239](https://github.com/status-im/status-desktop/pull/19239) Appium 1x1 chat tests; awaiting review.
- `status:2025q4-status-qa-mobile:release-testing`
  - Minor retests.
- [`vac:qa:status:2025q4-status-qa-mobile:test-ci`](/qa/status/2025q4-status-qa-mobile#android-ci-workflow--test)
  - Addressed second round of Jenkinsfile feedback.
- `admin/misc`
  - Prepared Q1 plan.
  - OOO: 3 CC days.

### vac:rfc:
- [`vac:rfc:ift:2025q4-rfc-index-maintenance:maintenance`](/rfc/ift/2025q4-rfc-index-maintenance#maintenance)
  - Responded to open reviews.
- [`vac:rfc:waku:2025q4-rfc-iteration:opchan`](/rfc/waku/2025q4-rfc-iteration#opchan)
  - Draft in progress; aiming for review-ready next week.
- `vac:rfc:nomos:2025q4-rfc-iteration:message-encapsulation`
  - [O] [rfc-index PR #218](https://github.com/vacp2p/rfc-index/pull/218) draft opened.
- `vac:rfc:nomos:2025q4-rfc-iteration:message-formatting`
  - [O] [rfc-index PR #219](https://github.com/vacp2p/rfc-index/pull/219) draft opened.
- `vac:rfc:nomos:2025q4-rfc-iteration:blend-protocol`
  - [O] [rfc-index PR #213](https://github.com/vacp2p/rfc-index/pull/213) updated.
- `vac:rfc:nomos:2025q4-rfc-iteration:proof-of-quota`
  - [O] [rfc-index PR #217](https://github.com/vacp2p/rfc-index/pull/217) updated.
- `vac:rfc:nomos:2025q4-rfc-iteration:codex-prover`
  - [M] [rfc-index PR #205](https://github.com/vacp2p/rfc-index/pull/205) feedback addressed.
- `vac:rfc:nomos:2025q4-rfc-iteration:block-exchange`
  - Responded to improvement feedback in [PR #215](https://github.com/vacp2p/rfc-index/pull/215).

### vac:sc:
- [`vac:sc:status-l2:2025q4-maintaining-status-l2-contracts:maintenance`](/sc/status-l2/2025q4-maintaining-status-l2-contracts#contract-maintenance-recurring)
  - Landed StakeManager pause/leave fix [PR #87](https://github.com/status-im/status-network-monorepo/pull/87) and StakeVault self-migration guard [PR #86](https://github.com/status-im/status-network-monorepo/pull/86); continued audit review.
  - Reviewed audit findings, paired on bug confirmations, and opened [PR #85](https://github.com/status-im/status-network-monorepo/pull/85) to enforce correct vault owners; started design for vault count limits ([issue #73](https://github.com/status-im/status-network-monorepo/issues/73)).

### vac:sec:
- `vac:sec:ift:2025q4-awareness-program:web3-security-essentials`
  - Reviewed and updated OS hardening guidelines.
- `vac:sec:ift:2025q4-secure-signing-process-training:signatory-onboarding-run`
  - Continued onboarding legacy members.
- `vac:sec:ift:2025q4-secure-signing-env-deployment:build-a-secure-vm-for signing`
  - Continued testing real transaction executions (adding new owners).
- `vac:sec:ift:2025q4-secure-signing-environment:endpoint-security-evaluation`
  - Began AVG and ClamAV evaluation.
- `vac:sec:ift:2025q4-secure-signing-environment:build-cold-signing-workstation`
  - Deployed a single-boot Qubes OS workstation (x86).
- `vac:sec:ift:2025q4-treasury-continuity-plan:implement-backup-requirements`
  - Updated device assignment matrix and started IR procedures for UI outages.
- `vac:sec:ift:2025q4-treasury-continuity-plan:signing-accounts-revalidation`
  - Started annual signing account revalidation.
- `admin/misc`
  - Continued refining 2026-Q1 commitments and tasks.
- `vac:sec:ift:2025q4-cicd-security-review:status-design-reviews`
  - Reviewed Status Desktop/Wallet PRs for crypto/sandbox safety, audited repos for insecure patterns, verified npm malware exposure, checked permissions, regenerated SBOMs, and validated dependency integrity.
- `vac:sec:ift:2025q4-vulma-and-ir:incidents`
  - Investigated npm malware indicators, swept repos/builds for compromise, monitored alerts, and confirmed secret rotations.
- `vac:sec:ift:2025q4-vulma-and-ir:remediation-tracking`
  - Deep security pass on Waku/VAC code, patched risky dependencies, ran CodeQL on routing/serialization/network surfaces, enforced pinned deps and signature validation, and coordinated fixes.

### vac:tke:
- `vac:tke:ift:support-to-ift-units::price-prediction`
  - Addressed latest comments and refreshed functions.
- `vac:tke:ift:logos-token:logos-digital-twin`
  - Began Machinations work, designed the digital twin, and started stock/flow diagrams.
- `vac:tke:ift:support-to-ift-units:reading-by-jarrad`
  - Read on virtual worlds and economies.

### vac:web:
- [`vac:web:status:2025q4-status-wallet-extension:price-proxy`](/web/status/2025q4-status-wallet-extension#price-proxy)
  - [M] [status-web PR #860](https://github.com/status-im/status-web/pull/860) reviewed proxies and data.
- [`vac:web:status:2025q4-sn-hub`](/web/status/2025q4-sn-hub#staking)
  - [O] [status-web PR #827 commit](https://github.com/status-im/status-web/pull/827/commits/ff003fcb19f872163de3975740381e9b8ca60e17) deploys Status proxy; [O] [PR #827](https://github.com/status-im/status-web/pull/827) reviews depositing; [M] [PR #873](https://github.com/status-im/status-web/pull/873) improves responsiveness and multiple hub sections; [M] [PR #816](https://github.com/status-im/status-web/pull/816) adds predeposit UI.
- `admin/misc`
  - Off 2 days; planned CRM tasks [issue #80](https://github.com/status-im/infra-office/issues/80); prioritized wallet extension tasks ([roadmap PR #345](https://github.com/vacp2p/roadmap/pull/345/files#diff-b1393e315ba726c2827a2335aea2675cbac6f4fd91e92060076c533a9982ecd3)); tested ClickUp integration; budgeted Logos website service cost.
  - OOO: 3 CC days.
- [`vac:web:status:2025q4-sn-hub:deposit`](/web/status/2025q4-sn-hub#deposit)
  - Added TVL to vaults via [PR #816](https://github.com/status-im/status-web/pull/816).
- [`vac:web:status:2025q4-sn-hub:discovery`](/web/status/2025q4-sn-hub#discovery)
  - [M] [PR #873](https://github.com/status-im/status-web/pull/873) covers Discover columns, Karma tiers/progress, staking flow locks, vault visibility rules, and token availability fixes.
- [`vac:web:ift:2025q4-maintenance:maintenance`](/web/ift/2025q4-maintenance#maintenance)
  - [O] [PR #872](https://github.com/status-im/status-web/pull/872) adds CI for non-Vercel contributors; addressed shard changes for Communities in [community-dapp PR #119](https://github.com/status-im/community-dapp/pull/119) and [status-web PR #875](https://github.com/status-im/status-web/pull/875); tested multiple deployment and API endpoint updates.
- [`vac:web:status:2025q4-status-app:upgrade-waku`](/web/status/2025q4-status-app#upgrade-waku)
  - Prepared shards update in [status-web PR #875](https://github.com/status-im/status-web/pull/875).
- [`vac:web:status:2025q4-status-wallet-extension:token-proxy`](/web/status/2025q4-status-wallet-extension#token-proxy)
  - [O] [status-web PR #877](https://github.com/status-im/status-web/pull/877) fixes incorrect wallet data.
- [`vac:web:ift:2025q4-logos-website:layout`](/web/ift/2025q4-logos-website#layout)
  - [M] [logos PR #17](https://github.com/acid-info/logos/pull/17) adds Luma calendar embed; upgraded Next.js for CVE mitigation in [commit](https://github.com/acid-info/logos/commit/9e8716be86bdf6b98b1ea64703aeeed5fc5a8cf5).
- [`vac:web:ift:2025q4-logos-website:spaces-calendar`](/web/ift/2025q4-logos-website#spaces-calendar)
  - [O] [admin-acid.logos.co PR #25](https://github.com/acid-info/admin-acid.logos.co/pull/25/commits) adds horizontal scrolling; metrics fields added in [commit](https://github.com/acid-info/admin-acid.logos.co/pull/25/commits/5d8c88cf13cad618d392d84768d618c51f1d4399).
- [`vac:web:ift:2025q4-psf-website:website`](/web/ift/2025q4-psf-website#website)
  - [O] UI/media updates in [ps.logos.co PR #13](https://github.com/acid-info/ps.logos.co/pull/13) ([preview](https://psf-logos-co-git-background-acidinfo.vercel.app/)).
- [`vac:web:status:2025q4-sn-hub:feedback`](/web/status/2025q4-sn-hub#feedback)
  - [M] [status-web PR #876](https://github.com/status-im/status-web/pull/876) UI updates from Figma.
- [`vac:web:ift:2025q4-maintenance:maintenance`](/web/ift/2025q4-maintenance#maintenance)
  - [M] [status-software-legal-documents PR #21](https://github.com/status-im/status-software-legal-documents/pull/21) updates privacy policy; addressed Next.js CVEs across Logos.co, Press Engine, and Contribute Portal; synced admin-acid environments with infra.

### vac:bi:
- Team mostly Out of office
- Web Tracking: issue analysis for Keycard Shopify analytics
- LLM: research on vLLM to replace Ollama
